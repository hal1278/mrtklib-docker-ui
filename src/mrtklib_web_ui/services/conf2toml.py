"""conf2toml — Convert MRTKLIB legacy .conf files to v0.7.6 TOML format.

Vendored from MRTKLIB `scripts/tools/conf2toml.py`
(commit 2c83abb, 2026-07-12). Keep the MAPPING / ENUM_DEFS / conversion
logic in sync with upstream — those tables are the source of truth for the
v0.7.6 config schema. The only local changes are string-oriented helpers
(`parse_conf_text`, `dumps`, `convert_text`) added for use from the web API,
and the removal of the standalone argparse CLI.
"""
...
def write_toml(sections: dict[str, OrderedDict], out_path: Path, source_name: str = "") -> None:
    ...
    out_path.write_text(dumps(sections, source_name=source_name), encoding="utf-8")
...
def parse_conf_text(content: str) -> dict[str, OrderedDict]:
    """Parse .conf file *content* and return TOML sections dict.
...
# ═══════════════════════════════════════════════════════════════════════════════
# String-oriented API (web backend)
# ═══════════════════════════════════════════════════════════════════════════════


def dumps(sections: dict[str, OrderedDict], source_name: str = "") -> str:
    """Render a nested section dict as a TOML string (same layout as write_toml)."""
    lines: list[str] = []
...
def convert_text(content: str, source_name: str = "") -> str:
    """Convert legacy .conf *content* to a v0.7.6 TOML string."""
    sections = parse_conf_text(content)
    return dumps(sections, source_name=source_name)
